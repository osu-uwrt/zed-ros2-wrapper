...
void ZedCamera::initPublishers()
{
  ...
  std::string normals_topic = mTopicRoot + "normals";
  ...
  if (!mDepthDisabled) {
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Advertised mPubDepthInfoon topic: " << mPubDepth.getTopic());
  ...
  if (!mNormalsDisabled) {
    mPubNormals =
      image_transport::create_camera_publisher(this, normals_topic, mDepthQos.get_rmw_qos_profile());
      RCLCPP_INFO_STREAM(get_logger(), "Advertised on topic: " << mPubNormals.getTopic());
    RCLCPP_INFO_STREAM(get_logger(), "Advertised on topic: " << mPubNormals.getInfoTopic());
  }
...
bool ZedCamera::startCamera()
{
  ...
  mNormalsCamInfoMsg = std::make_shared<sensor_msgs::msg::CameraInfo>();
  ...
  mNormalsCamInfoMsg = mLeftCamInfoMsg;
  // <---- Camera Info messages
...
bool ZedCamera::areVideoDepthSubscribed()
{
  ...
  try {
    ...
    if (!mDepthDisabled) {
      ...
      mNormalsSubnumber = mPubNormals.getNumSubscribers();
...
void ZedCamera::retrieveVideoDepth()
{
  ...
  if (mNormalsSubnumber > 0) {
    RCLCPP_DEBUG(get_logger(), "Retrieving Normals");
    retrieved |=
      sl::ERROR_CODE::SUCCESS ==
      mZed.retrieveMeasure(mMatNormals, sl::MEASURE::NORMALS, sl::MEM::CPU, mMatResol);
    mGrabTS = mMatNormals.timestamp;
  }
...
void ZedCamera::publishVideoDepth(rclcpp::Time & out_pub_ts)
{
  ...
  // <----  Publish the normal map if someone has subscribed to
  if (mNormalsSubnumber > 0) {
    publishNormals(mMatNormals, timeStamp);
  }
  // <----  Publish the normal map if someone has subscribed to
...
void ZedCamera::publishNormals(sl::Mat & normals, rclcpp::Time t)
{
  mNormalsCamInfoMsg->header.stamp = t;
  auto normals_map = sl_tools::imageToROSmsg(normals, mDepthOptFrameId, t);
  mPubNormals.publish(normals_map, mNormalsCamInfoMsg);
  return;
}
